...
from .serializers import (
    PostSerializer,
    CliqueSerializer,
    PostSerializer_detailed,
    CurrentCliqueSerializer,
    CliqueSerializer_detailed,
...
class CliqueRetrieveUpdateDeleteView(
    generics.GenericAPIView,
    mixins.RetrieveModelMixin,
    mixins.UpdateModelMixin,
    mixins.DestroyModelMixin,
    ...
    def perform_create(self, serializer):
        ...
        if CommentPost.objects.filter(post=post, occupier=self.request.user).exists():
            raise serializers.ValidationError(
                {"Message": "You have already added comment on this post"}
            )
