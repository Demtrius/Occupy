...
async def test_slots_returns_empty_for_wrong_weekday(db_session: AsyncSession):
    ...
    # Setup: Create a user and clique
    user = User(id=uuid4(), email="test@example.com", username="testuser", full_name="Test User")
    clique = Clique(id=uuid4(), name="Test Clique", owner_user_id=user.id)
    service = Service(
        id=uuid4(),
        title="Test Service",
        clique_id=clique.id,
    ...
    # Create availability for Tuesday (day_of_week=1)
    ...
    db_session.add_all([user, clique, service, availability])
    ...
    for slot in slots:
        start_ts = datetime.fromisoformat(slot["start_ts"])
        end_ts = datetime.fromisoformat(slot["end_ts"])
        
        # Should be on Tuesday in UTC
        assert start_ts.date() == date(2025, 11, 4)
        assert end_ts.date() == date(2025, 11, 4)
        
        # Should be within 10:00-18:00 UTC (accounting for 1-hour duration)
        assert start_ts.hour >= 10
        assert end_ts.hour <= 18
        
        # Should be 1 hour duration
        assert end_ts - start_ts == timedelta(hours=1)
...
async def test_slots_timezone_handling(db_session: AsyncSession):
    ...
    # Create availability for Tuesday in America/New_York timezone
    availability = Availability(
        id=uuid4(),
        clique_id=clique.id,
        is_recurring=True,
    ...
    while current_day <= date_end:
        ...
        if availability.is_recurring:
            if availability.day_of_week is None:
                ...
                print(f"    -> Skipping: weekday {current_day.weekday()} != {availability.day_of_week}")
            else:
