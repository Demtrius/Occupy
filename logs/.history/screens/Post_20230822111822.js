...
const {loggedIn,setIsLoggedIn,occupierObj,setOccupierObj,token,setToken,authTokens,setAuthTokens,setItem} = globalContext
...
class CreatePost extends React.Component {
    ...
    handleSubmit(context) {
        let json = {
            ...
            occupier: Number(this.state.occupier.id)
        ...
        .then(response => {
            response.ok ? response.json().then( json => this.success()) : console.log(response)
        })
    }
render(){
    return (
        <View>
            <TextInput
            type='select'
            onChange={this.handleCliqueSelect}
            />
            <option id="options" key='0' disabled selected> Select an Option</option>
            {this.state.clique.map((clique) => {
                                                return (
                                                    <option key={clique.id} id={clique.id}>{clique.name}</option>
                                                )
                                            })}
