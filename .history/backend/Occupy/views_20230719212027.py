...
class CliqueViewSet(viewsets.ViewSet):
    ...
    def posts(self,request,pk=None):
        clique = self.get_object()
        queryset = Post.objects.filter(clique=clique)
        serializer_class = self.get_serializer_class()
        serializer = serializer_class(queryset, many=True, context={'user': request.occupier })
        return Response(serializer.data, status=status.HTTP_200_OK)
