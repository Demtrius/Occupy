...
from rest_framework import fields, serializers,ModelReadOnlySerializer
...
class CliqueHeavySerializer(serializers.ModelSerializer):
    topics = TagSerializer(required=False,many=True)
    members_count = serializers.SerializerMethodField
    clique_level = serializers.SerializerMethodField
    member_status = serializers.SerializerMethodField

    class Meta:
        model = Clique
        fields = ('id','name','description','clique_level','topics','member_status','members_count','created_at')
...
class PostSerializer(serializers.ModelSerializer):
    clique = CliqueReadOnlyLightSerializer()
    occupier = Occupier()
