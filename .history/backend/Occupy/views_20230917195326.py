...
from .serializers import PostSerializer,CliqueSerializer,PostSerializer_detailed,CurrentCliqueSerializer,CliqueSerializer_detailed,CommentSerializer
from Occupier.serializers import CurrentOccupierSerializer
from .models import  Post,Clique,Comment
...
class CommentListCreateView(generics.ListAPIView):
    ...
    def perform_create(self,serializer):
        ...
        if Comment.objects.filter(post=post, occupier=self.request.occupier).exists():
            raise serializers.ValidationError({'Message': 'You have already added comment on this post'})
        serializer.save(occupier=self.request.occupier, post=post)
