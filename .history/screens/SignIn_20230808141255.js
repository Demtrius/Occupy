...
function SignIn({ navigation,route,props }){
  ...
  const { setIsLoggedIn,occupierObj,setOccupierObj,setToken,authTokens,setAuthTokens} = globalContext
 ...
 let handleLogin = async () => {
  ...
  .then((response) => {
    ...
    AsyncStorage.setToken('token',token)
    let user_id = jwt_decode(response.data.access).user_id;
    localStorage.setToken("user_id", user_id);
    return response
  })
