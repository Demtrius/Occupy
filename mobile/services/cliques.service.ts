...
class CliquesService {
	...
	async getAllCliques(
		page: number = 1,
		limit: number = 20,
	): Promise<PaginatedResponse<Clique>> {
		try {
			const response =
				await apiHelpers.get<PaginatedResponse<Clique>>("/api/v1/cliques/");
			return response;
		} catch (error) {
	...
	async getCliqueById(cliqueId: number): Promise<Clique> {
		try {
			const clique = await apiHelpers.get<Clique>(
	...
	async getMyCliques(): Promise<Clique[]> {
		try {
			const cliques = await apiHelpers.get<Clique[]>("/api/v1/cliques/me/");
			return cliques;
		} catch (error) {
