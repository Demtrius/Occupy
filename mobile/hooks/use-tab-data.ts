		...
		async (tab: TabType) => {
			try {
				switch (tab) {
					case "Services":
						const servicesData =
							await servicesService.getCliqueServices(cliqueId);
						setServices(servicesData);
						break;
					case "Availability":
						const availabilityData =
							await availabilityService.getCliqueAvailability(cliqueId);
						setAvailability(availabilityData);
						break;
					case "Bookings":
						if (isOwner) {
							const bookingsData =
								await bookingService.getCliqueBookings(cliqueId);
