...
const MessageDetail: React.FC<Props> = ({ route, navigation }) => {
  ...
  const renderMessage = ({ item, index }: { item: Reply; index: number }) => {
    ...
    const showTimestamp = index === 0 ||
      (index > 0 && new Date(item.createdAt).getTime() - new Date(conversation!.replies[index - 1].createdAt).getTime() > 300000);
    ...
    >
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.headerAction}>
          <Ionicons name="information-circle-outline" size={24} color="#6ba32d" />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          style={[
            styles.sendButton,
            (newMessage.trim().length === 0 || sending) && styles.sendButtonDisabled,
          ]}
          onPress={sendMessage}
