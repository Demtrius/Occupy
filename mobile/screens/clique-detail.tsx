...
import { cliquesService, postsService, bookingService } from '../services';
import { showError, showSuccess } from '../store/app.store';
import { useAuthStore } from '../store/auth.store';
import { Clique, Post, Service, Availability, Booking } from '../types';
import { RootStackParamList } from '../types';

const { width, height } = Dimensions.get('window');

type CliqueDetailScreenRouteProp = RouteProp<RootStackParamList, 'CliqueDetail'>;
type CliqueDetailScreenNavigationProp = StackNavigationProp<RootStackParamList>;

interface Props {
  route: CliqueDetailScreenRouteProp;
}

type TabType = 'About' | 'Services' | 'Availability' | 'Bookings' | 'Posts' | 'Reviews';

const CliqueDetailScreen: React.FC<Props> = ({ route }) => {
  const navigation = useNavigation<CliqueDetailScreenNavigationProp>();
  ...
  useEffect(() => {
    if (clique && user) {
      ...
      const createdById = typeof clique.createdBy === 'object' ? clique.createdBy?.id : clique.createdBy;
  ...
  const loadTabData = useCallback(async (tab: TabType) => {
    try {
      switch (tab) {
        case 'Services':
          const servicesData = await bookingService.getCliqueServices(id);
          setServices(servicesData);
          break;
        case 'Availability':
          const availabilityData = await bookingService.getCliqueAvailability(id);
  ...
  const handleBookService = (service: Service) => {
    ...
    (navigation as any).navigate('BookingCreate', {
      serviceId: service.id,
    });
  };

  const handleAddService = () => {
    (navigation as any).navigate('ServiceCreate', { cliqueId: id });
  };

  const handleAddAvailability = () => {
    (navigation as any).navigate('AvailabilityCreate', { cliqueId: id });
  };
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => {
                (navigation as any).navigate('BookingDetail', { id: item.id });
              }}
            >
              <View style={styles.bookingHeader}>
                <View>
                  <Text style={styles.bookingService}>
                    {typeof item.service === 'object' ? item.service.title : 'Service'}
                  </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => {
                (navigation as any).navigate('PostDetail', { id: item.id });
              }}
            >
