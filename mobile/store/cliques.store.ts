				...
				cliques: state.cliques.map((clique) =>
					clique.id === cliqueId
						? {
								...clique,
								isMember: true,
								membersCount: (clique.membersCount || 0) + 1,
							}
						: clique,
				),
				...
				cliques: state.cliques.map((clique) =>
					clique.id === cliqueId
						? {
								...clique,
								isMember: false,
								membersCount: Math.max((clique.membersCount || 0) - 1, 0),
							}
						: clique,
				),
				filteredCliques: state.filteredCliques.map((clique) =>
	...
	deleteClique: async (cliqueId: number) => {
		try {
			...
			set((state) => ({
				...
				filteredCliques: state.filteredCliques.filter(
					(clique) => clique.id !== cliqueId,
				),
			}));
		} catch (error) {
