...
interface BookingsState {
  ...
  updateBooking: (bookingId: number, data: Partial<CreateBookingData>) => Promise<void>;
...
export const useBookingsStore = create<BookingsState>((set, get) => ({
  ...
  confirmBooking: async (bookingId) => {
    try {
      ...
      set((state) => ({
        bookings: state.bookings.map(booking =>
          booking.id === bookingId ? { ...booking, status: "confirmed" as BookingStatus } : booking
        ),
      }));
    } catch (error) {
  ...
  completeBooking: async (bookingId) => {
    try {
      ...
      set((state) => ({
        bookings: state.bookings.map(booking =>
          booking.id === bookingId ? { ...booking, status: "completed" as BookingStatus } : booking
        ),
      }));
    } catch (error) {
